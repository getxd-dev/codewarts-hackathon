...
This repository uses the IBM-inspired `getdesign` contract as a design reference for software-factory examples. Do not use IBM logos, wordmarks, product names, or official brand assets unless you have the right to do so. Keep implementations focused on the Carbon-like grid, typography, blue accent, square geometry, and restrained enterprise UI language.
